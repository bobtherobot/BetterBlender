...
class MaskToVertexGroup(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if context.active_object.mode == 'SCULPT' :
           ...
           mask = bmeshContainer.verts.layers.paint_mask.active # Set the active mask layer as custom layer
           
           newVertexGroup = context.active_object.vertex_groups.new(name = "Mask") # Create an empty vgroup
           ...
           for x in bmeshContainer.verts:  # itterate from bmesh.verts
               ...
               if x[mask] > 0 : # if x BMVert has mask weight
                   ...
                   newVertexGroup.add([x.index,x.index],maskWeight,"REPLACE") # add it to vgroup, set mask weight                       
               else :
                   
                   newVertexGroup.add([x.index,x.index],0,"REPLACE")
            ...
            if vGroupLocked == False : 
               ...
               for x in bmeshContainer.verts:  # itterate from bmesh.verts
                   ...
                   if x[mask] > 0 : # if x BMVERT has mask weight
                       ...
                       activeVertexGroup.add([x.index,x.index],(maskWeight * 0.5),"ADD") # add it to vgroup, set mask weight
            ...
            if vGroupLocked == False :
               ...
               for x in bmeshContainer.verts:  # itterate from bmesh.verts
                   ...
                   if x[mask] > 0 : # if x BMVert has mask weight
                       ...
                       activeVertexGroup.add([x.index,x.index],(maskWeight * 0.5),"SUBTRACT") # add it to vgroup, set mask weight
